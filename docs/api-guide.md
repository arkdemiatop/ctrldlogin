...
The backend exposes a REST API + WebSocket server. All endpoints return JSON and support optional Bearer token authentication via `Authorization: Bearer <token>` header.
